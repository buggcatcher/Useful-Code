...
let lastImageGeoData = null;

document.addEventListener('DOMContentLoaded', function() {
    fetch('/assets')
        .then(response => response.text())
        .then(data => {
            // Parse the directory listing HTML to find image files
            const parser = new DOMParser();
            const doc = parser.parseFromString(data, 'text/html');
            const links = Array.from(doc.querySelectorAll('a'));
            const imageFiles = links.map(link => link.getAttribute('href')).filter(href => /\.(jpg|jpeg|png|gif)$/i.test(href));
                ...
                .then(blob => {
                    ...
                    img.onload = function() {
                        EXIF.getData(img, function() {
							...
							lastImageGeoData = { dateTime, lat, long };
