document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("addPhotoButton").addEventListener("click", function() {
        if (navigator.geolocation) navigator.geolocation.getCurrentPosition(async function(position) {
            var latlng = [
    ...
    map.on("zoomend", function() {
        ...
        if (currentZoom >= 15) addMarkers(); // Add markers when zoomed in
        else removeMarkers(); // Remove markers when zoomed out
    });
});
...
async function createPhoto(localita, timestamp, photo) {
    const result = await pool.query(`
        INSERT INTO photos (localita, timestamps, immagine) VALUES (?, ?, ?)`, [
        localita,
        timestamp,
        photo
    ]);
